...
{
	Vector3 origin = mObj->mObjectData.c_modelToWorld.GetTranslation();
	Vector3* point = new Vector3();
	Physics::Ray ray = Physics::Ray(origin + Vector3(0,0,10), origin - Vector3(0,0,10));
	if (mObj->GetGame()->mPhysics->RayCast(ray, point)) {
		*groundHeight = point->z;
		return true;
	}
    return false;
}
